	...
	SDL_Renderer* renderer, char* text, Colors& colors)
{
	...
	sprintf_s(text, 128, "Mario lifes = %d		Mario points = %d ", mario_info.lifes, mario_info.points);
	DrawString(screen, screen->w / 2 - strlen(text) * 8 / 2, 10, text, charset);
	sprintf_s(text, 128, "Esc - start the game, \030 - choose higher level, \031 - choose lower level");
